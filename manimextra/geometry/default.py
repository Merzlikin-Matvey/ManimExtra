...
class Line(manim.Line):
    def __init__(self, start: Point3DLike = LEFT, end: Point3DLike = RIGHT, **kwargs):
    ...
    def set_length_about_point(self, length: float, dot: Point3DLike):
        dot = dot_to_array(dot)[0]
    ...
    def equal(self, n: int = 1, buff: float = 0.1, length: float = 0.2, **kwargs):
    ...
    def paral(self, n: int = 1, buff: float = 0.08, length: float = 0.15, rotate: bool = False, **kwargs):
        elem = manim.VGroup(
...
class Circle(manim.Circle):
    ...
    @staticmethod
    def from_three_points(p1: Point3DLike, p2: Point3DLike, p3: Point3DLike, **kwargs):
