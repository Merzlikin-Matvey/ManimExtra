...
class Line(manim.Line):
    ...
    def is_point_in_line(self, dot: Point3DLike):
    ...
    def set_length_about_point(self, length, dot: Point3DLike):
    ...
    def get_distance(self, dot: Point3DLike):
    ...
    def equal(self, n=1, buff=0.1, length=0.2, **kwargs):
        return manim.VGroup(*[Line(**kwargs).set_length(length).rotate(
            about_point=Line(**kwargs).set_length(length).get_center(), angle=np.pi / 2) for _ in range(n)]).arrange(
            buff=buff, direction=RIGHT).move_to(self.get_center()).rotate(
            about_point=self.get_center(), angle=self.get_angle()).set_z_index(1)

    def parallel(self, n=1, buff=0.08, length=0.15, rotate=False, **kwargs):
        elem = manim.VGroup(
        ...
        return manim.VGroup(*[elem.copy() for _ in range(n)]).arrange(buff=buff, direction=RIGHT).move_to(
            self.get_center()).rotate(about_point=self.get_center(), angle=self.get_angle()).set_z_index(1)

    @deprecated(since="v2.0.0", message="Use `Line.parallel` instead.")
...
class Circle(manim.Circle):
    ...
    def pow(self, dot: Point3DLike):
        value = (pow(self.get_center()[0] - dot[0], 2) +
                 pow(self.get_center()[1] - dot[1], 2)) - self.radius ** 2
        return np.round(value, 4)

    def is_point_in_circle(self, dot: Point3DLike):
        return self.pow(dot) == 0

    def get_diametrically_opposite_point(self, dot: Point3DLike):
